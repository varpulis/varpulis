...
.PHONY: verify verify-fmt verify-clippy verify-audit verify-deny verify-doc help

# Run the full local verification suite (fmt + clippy + audit + deny + doc).
...
verify-fmt:
	@SKIP_CLIPPY=1 SKIP_AUDIT=1 SKIP_DENY=1 SKIP_DOC=1 bash scripts/verify.sh

verify-clippy:
	@SKIP_FMT=1 SKIP_AUDIT=1 SKIP_DENY=1 SKIP_DOC=1 bash scripts/verify.sh

verify-audit:
	@SKIP_FMT=1 SKIP_CLIPPY=1 SKIP_DENY=1 SKIP_DOC=1 bash scripts/verify.sh

verify-deny:
	@SKIP_FMT=1 SKIP_CLIPPY=1 SKIP_AUDIT=1 SKIP_DOC=1 bash scripts/verify.sh
...
help:
	@echo "Targets:"
	@echo "  make verify          Run fmt + clippy + audit + deny + doc (full local suite)"
	...
	@echo "  make verify-doc      Run only cargo doc with -D warnings (intra-doc links)"
