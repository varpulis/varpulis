...
mod tests {
    ...
    #[test]
    fn test_compare_parsers_followed_by() {
        let source =
            r#"stream Test = A as a -> B where value == a.base + 10 as b .emit(status: "matched")"#;

        let pest_result = parse(source);
        let old_result = crate::parser::parse(source);

        assert!(pest_result.is_ok(), "Pest failed: {:?}", pest_result.err());
        assert!(old_result.is_ok(), "Old failed: {:?}", old_result.err());

        // Compare AST structures
        let pest_ast = pest_result.unwrap();
        let old_ast = old_result.unwrap();
    ...
    #[test]
    fn test_compare_parsers_subtraction() {
        let source =
            r#"stream Test = A as a -> B where value == a.base - 3 as b .emit(status: "matched")"#;
    ...
    #[test]
    fn test_compare_parsers_match_all() {
        let source = r#"stream AllTicks = News as news -> all Tick as tick .emit(matched: "yes")"#;
    ...
    #[test]
    #[ignore] // Known limitation: pest parser doesn't handle indentation-based function blocks
    fn test_compare_parsers_function_emit() {
        let source = r#"
        ...
        let pest_str = format!("{:?}", pest_ast);
        let old_str = format!("{:?}", old_ast);

        if pest_str != old_str {
            println!("PEST: {:#?}", pest_ast);
            println!("OLD: {:#?}", old_ast);
        }

        assert_eq!(pest_str, old_str, "ASTs differ!");
    }
}
