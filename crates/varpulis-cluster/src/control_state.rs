//! Backend-agnostic coordinator control-plane state and command application.
//!
//! This module holds the *data* of the cluster control plane — the worker
//! registry, pipeline groups, connectors, migrations, models, per-worker
//! metrics and per-group checkpoint status — together with the command set
//! that mutates it and the pure [`apply_command`] function that applies one
//! command to one state value.
//!
...
//!   key, and the state is materialised by reading the bucket.
//!
//! The split exists because every one of the command variants below mutates
//! exactly **one** entity keyed by exactly **one** id — the only variant that
//! touches two collections ([`ClusterCommand::WorkerMetricsUpdated`]) writes
//! two facets of the same worker. That is what makes a per-key CAS store a
//! sufficient substrate and a total order unnecessary; the full argument is in
//! the `jetstream_control_plane` module docs.
...
/// The pure-data subset of coordinator state shared across coordinators.
...
pub struct WorkerEntry {
    ...
    /// replicated (via [`ClusterCommand::WorkerMetricsUpdated`]).
    /// `sync_from_control_state` treats a worker as freshly-alive only when
    /// this value advances, so a dead worker (no new heartbeats ⇒ seq frozen)
    /// is eventually timed out by the health sweep instead of being kept
    /// `Ready` forever. `#[serde(default)]` keeps pre-existing snapshots
    /// deserializable.
    #[serde(default)]
...
/// Commands that mutate the shared coordinator state.
///
/// Under the `raft` backend these are the replicated log entries. Under the
/// `jetstream-control-plane` backend each variant maps to a single-key
/// compare-and-swap.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum ClusterCommand {
    ...
    WorkerMetricsUpdated {
        ...
        /// once per received heartbeat; `sync_from_control_state` refreshes a
        /// worker's `last_heartbeat` only when this value moves forward.
        /// `#[serde(default)]` keeps older replicated/persisted commands
        /// deserializable.
        #[serde(default)]
        heartbeat_seq: u64,
    },
    ...
    /// the shared state store. Sharing this makes the new checkpoint id
    /// visible to every coordinator, which they use on recovery to decide
    /// which assembled snapshot to restore from.
    ...
    /// persistence failure, …). Shared for observability and so that other
    /// coordinators do not retry the same id.
    #[cfg(feature = "distributed-checkpoint")]
    CheckpointAborted {
...
/// Apply a single [`ClusterCommand`] to the shared state.
