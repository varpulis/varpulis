...
pub struct SaseEngine {
    ...
    pub(crate) partitioned_runs: FxHashMap<String, Vec<Run>>,
    ...
    pub(crate) max_timestamp: Option<DateTime<Utc>>,
    /// BP-01: Backpressure strategy
    backpressure: BackpressureStrategy,
    /// BP-01: Cumulative stats for backpressure
    pub(crate) total_runs_dropped: u64,
    pub(crate) total_runs_evicted: u64,
    pub(crate) total_runs_created: u64,
    pub(crate) total_runs_completed: u64,
...
impl SaseEngine {
    ...
    pub fn process(&mut self, event: &crate::event::Event) -> Vec<MatchResult> {
    ...
    pub fn process_with_result(&mut self, event: &crate::event::Event) -> ProcessResult {
        let shared_event = Arc::new(event.clone());
    ...
    pub fn process_instrumented(&mut self, event: &crate::event::Event) -> Vec<MatchResult> {
    ...
    fn update_watermark(&mut self, event: &crate::event::Event) {
    ...
    fn check_global_negations(&mut self, event: &crate::event::Event) {
