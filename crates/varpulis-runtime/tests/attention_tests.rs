...
fn test_embed_auto_simple_event() {
    ...
    let event = create_event("TestEvent", vec![("value", Value::Float(42.0))]);
...
fn test_embed_deterministic() {
    ...
    let event = create_event(
        "Trade",
        vec![("price", Value::Float(100.0)), ("volume", Value::Int(500))],
    );
...
fn test_embed_different_events_differ() {
    ...
    assert_ne!(
        emb1, emb2,
        "Different events should have different embeddings"
    );
}
...
fn test_embed_missing_configured_field() {
    ...
    let event = create_event("Trade", vec![("volume", Value::Int(1000))]);
...
fn test_transform_identity() {
    ...
    assert_eq!(
        engine.transform_numeric(42.0, &NumericTransform::Identity),
        42.0
    );
    assert_eq!(
        engine.transform_numeric(-10.0, &NumericTransform::Identity),
        -10.0
    );
    assert_eq!(
        engine.transform_numeric(0.0, &NumericTransform::Identity),
        0.0
    );
}
...
fn test_attention_score_similar_vs_different() {
    ...
    let similar = create_event(
        "Trade",
        vec![("price", Value::Float(101.0)), ("volume", Value::Int(1010))],
    );

    let different = create_event(
        "Order",
        vec![
