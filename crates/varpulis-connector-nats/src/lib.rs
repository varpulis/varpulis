...
//!
//! This crate provides full NATS connectivity for the Varpulis CEP engine
//! via the `async-nats` library. It includes source connectors for subscribing
//! to NATS subjects and sink connectors for publishing events.
...
pub use managed::ManagedNatsConnector;
...
impl ConnectorFactory for NatsFactory {
    ...
    fn create_engine_sink(
        ...
        let nats_config = NatsConfig::new(&servers, &subject);
        let sink = NatsSink::new(name, nats_config);
...
pub(crate) fn build_connect_options(config: &NatsConfig) -> async_nats::ConnectOptions {
...
pub(crate) fn parse_nats_payload(payload: &str, subject: &str) -> Option<Event> {
...
fn json_value_to_native(v: &serde_json::Value, _depth: usize) -> varpulis_core::Value {
    varpulis_connector_api::helpers::json_to_value(v).unwrap_or(varpulis_core::Value::Null)
}
