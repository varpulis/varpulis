#!/bin/bash
# =============================================================================
# SASE Security Demo Startup Script
# =============================================================================
#
# Architecture:
...
#   2. SASE generator (Login, Transaction, Logout, PasswordChange events)
#   3. Varpulis CEP engine (processes sase/main.vpl)
#   4. Dashboard UI (optional)
#
# Usage:
#   ./start-sase.sh           # Start all components
#   ./start-sase.sh --no-ui   # Start without dashboard
#   ./start-sase.sh --stop    # Stop all components
#
# =============================================================================
...
stop_demo() {
    log_info "Stopping SASE demo..."
    pkill -f "varpulis run.*sase" 2>/dev/null || true
...
echo ""
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║           🔐 VARPULIS SASE SECURITY DEMO                       ║"
echo "╚════════════════════════════════════════════════════════════════╝"
echo ""
...
# Step 2: Start SASE generator
log_info "Starting SASE event generator..."
docker compose -f "$SCRIPT_DIR/docker-compose.yml" run -d --rm \
    generator /generators/sase/generator.py --broker mosquitto --rate 2

sleep 1
log_success "Generator producing Login, Transaction, Logout events"
...
echo "  │    demos/sase/main.vpl                                  │"
echo "  │                                                         │"
echo "  │  Detecting patterns:                                    │"
echo "  │    - Account Takeover (login→pwd_change→large_txn)      │"
echo "  │    - Impossible Travel (logins from distant locations)  │"
echo "  │    - Fraud Pattern (transactions > $10,000)             │"
echo "  │    - Brute Force (failed logins > 5)                    │"
echo "  │                                                         │"
...
"$VARPULIS_BIN" run --file "$SCRIPT_DIR/sase/main.vpl"
